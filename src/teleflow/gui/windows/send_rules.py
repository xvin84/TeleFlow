...
class SendRulesDialog(QDialog):
    ...
    async def _load_chats(self) -> None:
        ...
        # get_assigned_chats returns: db_chat_id, chat_id (telegram), title, type, is_active
    ...
    async def _add_chat(self, chat_data: dict[str, Any]) -> None:
        ...
        # Check write permissions for channels
    ...
    async def _check_can_send(self, tg_chat_id: int) -> bool:
        ...
        try:
            ...
            # If we can't check permissions, allow it — sender_engine will handle the error
            return True
    ...
    async def _create_schedule(self, config: Any) -> None:
        ...
        # Save metadata to DB (done here in async context — no event loop conflicts)
